#!/usr/bin/env python3
"""
Sekai 场景基准测试 — DeepSeek 模型
====================================
基于 benchmark_sekai_kimi.py 适配 DeepSeek API。
...
用法:
  ...
  python benchmark_sekai_deepseek.py
  python benchmark_sekai_deepseek.py --models deepseek-v4-pro --rounds 3
"""
...
OUTPUT_DIR = Path(__file__).parent / "results" / "domestic" / "deepseek"
...
def call_deepseek_streaming(model: str, thinking_type: str, prompt_type: str, lang: str, max_tokens: int, timeout: int = 600) -> dict:
    ...
    try:
        with urllib.request.urlopen(req, context=ctx, timeout=timeout) as resp:
            ...
            for raw_line in resp:
                ...
                # DeepSeek streaming: reasoning_content for thinking, content for output
    ...
    if metrics["usage"]:
        u = metrics["usage"]
        # DeepSeek: completion_tokens = reasoning_tokens + content_tokens
        details = u.get("completion_tokens_details", {})
        reasoning = details.get("reasoning_tokens", 0) if details else 0
        total_completion = u.get("completion_tokens", 0)
        metrics["thinking_tokens"] = reasoning if reasoning > 0 else metrics["thinking_tokens"]
        metrics["content_tokens"] = total_completion - reasoning
        # DeepSeek: prompt_cache_hit_tokens + prompt_cache_miss_tokens
        cached = u.get("prompt_cache_hit_tokens", 0)
        missed = u.get("prompt_cache_miss_tokens", 0)
        metrics["prompt_cached_tokens"] = cached
        metrics["prompt_miss_tokens"] = missed
...
def summarize(results: list) -> dict:
    ...
    cached_toks = [r.get("prompt_cached_tokens", 0) for r in valid]
    miss_toks = [r.get("prompt_miss_tokens", 0) for r in valid]
    ...
    return {
        ...
        "prompt_cached_tokens": {"avg": round(statistics.mean(cached_toks)) if cached_toks else 0},
        "prompt_miss_tokens": {"avg": round(statistics.mean(miss_toks)) if miss_toks else 0},
    }
...
def main():
    parser = argparse.ArgumentParser(description="Sekai 场景基准测试 — DeepSeek")
    ...
    print(f"\n{'='*70}")
    print(f"Sekai 场景基准测试 (DeepSeek) — {total} 组合 × {args.rounds} 轮 = {total * args.rounds} 次调用")
    ...
    for idx, (model, prompt_type, thinking, lang) in enumerate(combos, 1):
        ...
        for rnd in range(1, args.rounds + 1):
            ...
            for attempt in range(1, 4):
                r = call_deepseek_streaming(model, thinking, prompt_type, lang, args.output_tokens)
                if not r["error"]:
                    break
                if attempt < 3:
                    wait = attempt * 5
            ...
            if r["error"]:
                ...
                if r.get("prompt_cached_tokens"):
                    parts.append(f"cached={r['prompt_cached_tokens']}tok")
                print(" | ".join(parts))
            time.sleep(2)
    ...
    result_file = OUTPUT_DIR / f"deepseek_benchmark_{ts}.json"
    ...
    print("汇总报告")
    print(f"{'='*140}")
    print(f"{'模型':<20} {'场景':<10} {'Think':<9} {'Lang':<5} {'E2E p50':>8} {'E2E p95':>8} {'Dec p50':>8} {'Dec p95':>8} {'TTFT':>6} {'ContT':>7} {'质量':>4} {'Cached':>7}")
    print("-" * 140)
    ...
    for key, data in all_results.items():
        ...
        c = data["config"]
        if s["rounds_valid"] == 0:
            print(f"{c['model']:<20} {c['prompt_type']:<10} {c['thinking']:<9} {c['lang']:<5} {'ERROR':>8}")
            continue
        print(
            f"{c['model']:<20} {c['prompt_type']:<10} {c['thinking']:<9} {c['lang']:<5} "
            ...
            f"{s['prompt_cached_tokens']['avg']:>7}"
        )
