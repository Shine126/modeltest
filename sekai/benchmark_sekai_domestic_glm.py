...
Sekai 场景基准测试 v2
=====================
模拟 Sekai（AI mini-app 平台）真实使用场景。

5 类测试 Prompt:
  1. code_gen - Mini-app 代码生成（记忆翻牌游戏）
  2. quiz     - 互动人格测试
  3. game     - 文字冒险游戏
  4. remix    - 代码改写（双人贪吃蛇）
  5. tool     - 实用工具（番茄钟）

测试矩阵: 3 models × 5 prompts × 2 thinking × 2 langs × N rounds

用法:
  export BIGMODEL_API_KEY="your-key"
  python benchmark_sekai.py
  python benchmark_sekai.py --models glm-5 --rounds 3
"""
...
OUTPUT_DIR = Path(__file__).parent / "results" / "domestic" / "glm"
...
def call_glm_streaming(model: str, thinking_type: str, prompt_type: str, lang: str, max_tokens: int, timeout: int = 600) -> dict:
    ...
    if metrics["usage"]:
        ...
        if "completion_tokens_details" in u:
            ...
            metrics["thinking_tokens"] = details.get("reasoning_tokens", metrics["thinking_tokens"])
            metrics["content_tokens"] = details.get("output_tokens", metrics["content_tokens"])
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Sekai 场景基准测试 v2")
    ...
    if not API_KEY:
        print("Error: set BIGMODEL_API_KEY env var", file=sys.stderr)
        return 1
    ...
    print(f"\n{'='*70}")
    print(f"Sekai 场景基准测试 — {total} 组合 × {args.rounds} 轮 = {total * args.rounds} 次调用")
    ...
    for idx, (model, prompt_type, thinking, lang) in enumerate(combos, 1):
        ...
        for rnd in range(1, args.rounds + 1):
            ...
            r = call_glm_streaming(model, thinking, prompt_type, lang, args.output_tokens)
    ...
    result_file = OUTPUT_DIR / f"sekai_benchmark_{ts}.json"
    # 不保存 content_sample 到最终 JSON（节省空间）
    ...
    print("汇总报告")
    print(f"{'='*130}")
    print(f"{'模型':<13} {'场景':<10} {'Think':<9} {'Lang':<5} {'E2E p50':>8} {'E2E p95':>8} {'Dec p50':>8} {'Dec p95':>8} {'TTFT':>6} {'ContT':>7} {'质量':>4}")
    print("-" * 130)
    ...
    for key, data in all_results.items():
        ...
        c = data["config"]
        if s["rounds_valid"] == 0:
            print(f"{c['model']:<13} {c['prompt_type']:<10} {c['thinking']:<9} {c['lang']:<5} {'ERROR':>8}")
            continue
        print(
            f"{c['model']:<13} {c['prompt_type']:<10} {c['thinking']:<9} {c['lang']:<5} "
