#!/usr/bin/env python3
"""
Sekai 海外节点基准测试 — GLM 模型
====================================
从北美/海外发起测试，测量 GLM-5 / GLM-5.1 的真实链路性能。
使用 Z.AI 海外端点 (api.z.ai)。

用法:
  export GLM_API_KEY="xxx.xxx"
  python3 benchmark_sekai_overseas_glm.py
  python3 benchmark_sekai_overseas_glm.py --rounds 1
...
def main():
    parser = argparse.ArgumentParser(description="Sekai 海外基准测试 — GLM")
    ...
    output_file = args.output or f"overseas_glm_{timestamp}.json"
    output_path = Path(__file__).parent / "results" / "overseas" / "glm" / output_file
    output_path.parent.mkdir(parents=True, exist_ok=True)

    output_data = {
        "meta": {
            "test_type": "overseas_glm",
            "timestamp": timestamp,
